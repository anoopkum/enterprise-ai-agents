resource "azurerm_machine_learning_workspace" "hub" {
  ...
  # Storage required by AML workspace
...
resource "azurerm_machine_learning_workspace" "project" {
  ...
  # Hub workspace ID links the project to the hub
  hub_id = azurerm_machine_learning_workspace.hub.id

  # Projects inherit storage/kv from hub — these are required by provider but unused
  storage_account_id      = azurerm_storage_account.hub.id
  application_insights_id = azurerm_application_insights.hub.id
  key_vault_id            = var.key_vault_id
...
# Supporting storage for the hub
...
# Application Insights for the hub
...
# Grant the project SP access to the OpenAI resource
